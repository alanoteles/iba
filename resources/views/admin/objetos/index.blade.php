...
@section('content')
    ...
    <div class="row">
        <div class="col-xs-12 align-right margin-top-10">
            <a href="{{ URL::to(App::getLocale() . '/admin/objetos/create') }}" class="btn btn-sm btn-success">Novo objeto</a>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table id="sample-table-1" class="table table-striped table-bordered table-hover no-margin">
                ...
                <tr>
                    ...
                    <th >Preâmbulo</th>
                    {{--<th>Autor</th>--}}
                    {{--<th class="hidden-480"  style="width: 90px;">Níveis</th>--}}
                    ...
                    @foreach($resultados as $key => $resultado)
                        ...
                        @if($resultado->locale == App::getLocale())
                            <tr>
                                ...
                                <td><a href="" data-route="/admin/objetos/{{ $resultado->id }}/edit">{{ $resultado->title }}</a></td>
                                <td><a href="" data-route="/admin/objetos/{{ $resultado->id }}/edit">{!!   $resultado->preamble !!}</a></td>
                                {{--<td>Fulano Beltrano Cicrano</td>--}}
                                {{--<td>--}}
                                    {{--<span>1</span> |--}}
                                    {{--<span>2</span> |--}}
                                ...
                                {{--</td>--}}
                                {{--<td class="align-left hidden-480 situacao situacao-clientes">Lorem </td>--}}
                                <td class="hidden-480 situacao situacao-clientes">
                                    <select class="form-control transparent status" name="status" style="height: 22px;"  data-id="{{ (isset($resultado->object_id) ? $resultado->object_id : $resultado->id) }}">
                                ...
                                <td class="acoes center">
                                    <div class="btn-group">
                                        <a href="#" class="btn btn-xs btn-grey remover-item-lista excluir"  data-id="{{ (isset($resultado->object_id) ? $resultado->object_id : $resultado->id)  }}">

jQuery(document).ready(function(){
  ...
  jQuery("form[name='frm']").on('submit', function(e){
    ...
    //   }
    // );
    var editorText = CKEDITOR.instances.explicacao.getData();

    if( editorText == "" ){
      mensagem += "<div> - Texto de introdução da página;</div>";
    }
  ...
  $( "#id-btn-dialog1" ).on('click', function(e) {
    ...
    var dialog = $( "#dialog-message" ).removeClass('hide').dialog({
      title: "Texto de introdução da página"
    });
    var editorText = CKEDITOR.instances.explicacao.getData();
    $("#dialog-message").html('');
    $("#dialog-message").html( editorText );
